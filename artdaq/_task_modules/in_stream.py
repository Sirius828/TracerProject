...
class InStream(object):
    ...
    def curr_read_pos(self):
        val = ctypes.c_uint64()
        cfunc = lib_importer.windll.ArtDAQ_GetReadCurrReadPos
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def avail_samp_per_chan(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetReadAvailSampPerChan
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def total_sampper_chan_acquired(self):
        val = ctypes.c_uint64()
        cfunc = lib_importer.windll.ArtDAQ_GetReadTotalSampPerChanAcquired
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def num_chans(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetReadNumChans
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def digital_bytes_perchan(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetReadDigitalLinesBytesPerChan
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
