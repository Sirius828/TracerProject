...
class OutStream(object):
    ...
    def space_avail(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetWriteSpaceAvail
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
                        lib_importer.task_handle, ctypes.POINTER(ctypes.c_uint32)]
    ...
    def total_samp_perchan_generated(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetWriteTotalSampPerChanGenerated
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
                        lib_importer.task_handle, ctypes.POINTER(ctypes.c_uint64)]
    ...
    def curr_write_pos(self):
        val = ctypes.c_uint64()
        cfunc = lib_importer.windll.ArtDAQ_GetWriteCurrWritePos
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def write_num_chans(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetWriteNumChans
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def digital_lines_bytes_perchan(self):
        val = ctypes.c_uint32()
        cfunc = lib_importer.windll.ArtDAQ_GetWriteDigitalLinesBytesPerChan
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
