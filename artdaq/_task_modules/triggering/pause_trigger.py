...
class PauseTrigger(object):
    ...
    @property
    def anlg_lvl_edge_hyst(self):
        val = ctypes.c_double()
        cfunc = (lib_importer.windll.ArtDAQ_GetAnlgLvlPauseTrigHyst)
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
    ...
    @anlg_lvl_edge_hyst.setter
    def anlg_lvl_edge_hyst(self, val):
        cfunc = (lib_importer.windll.ArtDAQ_SetAnlgLvlPauseTrigHyst)
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.c_double]
    ...
    @anlg_lvl_edge_hyst.deleter
    def anlg_lvl_edge_hyst(self):
        cfunc = lib_importer.windll.ArtDAQ_ResetAnlgLvlPauseTrigHyst
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle]
