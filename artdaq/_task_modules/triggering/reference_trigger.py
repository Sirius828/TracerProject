...
class ReferenceTrigger(object):
    ...
    def disable_ref_trig(self):
        ...
        """
        cfunc = lib_importer.windll.ArtDAQ_DisableRefTrig
    ...
    @property
    def anlg_edge_hyst_hyst(self):
        val = ctypes.c_double()
        cfunc = (lib_importer.windll.ArtDAQ_GetAnlgEdgeRefTrigHyst)
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
    ...
    @anlg_edge_hyst_hyst.setter
    def anlg_edge_hyst_hyst(self, val):
        cfunc = (lib_importer.windll.ArtDAQ_SetAnlgEdgeRefTrigHyst)
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.c_double]
        error_code = cfunc(self._handle, val)
        check_for_error(error_code)

    @anlg_edge_hyst_hyst.deleter
    def anlg_edge_hyst_hyst(self):
        cfunc = lib_importer.windll.ArtDAQ_ResetAnlgEdgeRefTrigHyst
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle]
