...
class Errors(Enum):
    ...
    PRO_PERTY_NOT_SUPPORTED_WITH_CONFLICTING_PROPERTY = -201298
    ...
    UNKNOWN = -1
    DMCSCSL_SUCCESS = 0
    DMCSCSL_ERR_PALMemoryConfigurationFault = -50350    # 分配内存失败
    DMCSCSL_ERR_PALDeviceNotSupported = -50302   # 设备不支持
    DMCSCSL_ERR_PALDeviceUnknown = -50301   # 未知设备
    DMCSCSL_ERR_SerivesCanNotConnect = -229779  # 无法连接到服务
    DMCSCSL_ERR_SendSerivesData = -229778   #发送数据失败
    DMCSCSL_ERR_RecieveSerivesData = -229776   # 接收数据失败
    DMCSCSL_ERR_SerivesResponseError = -229775   # 服务应答错误，操作未正常完成
    DMCSCSL_ERR_DuplicateTask = -200089   # 重复的任务名
    DMCSCSL_ERR_DeviceNotExist = -200220   # 设备不存在
    DMCSCSL_ERR_FileNotFound = -26103   # 文件找不到
    DMCSCSL_ERR_FileCannotAccess = -26104    # 文件无法访问
    DMCSCSL_ERR_DeviceAlreadyExist = -26105   # 设备已存在
    DMCSCSL_ERR_ReadBufferTooSmall = -200229   # 读取缓存区太小
    DMCSCSL_ERR_NULLPtr = -200604   # 参数为空
    DMCSCSL_ERR_InvalidTaskName = -201340   # 无效的任务名
    DMCSCSL_ERR_InvalidDeviceName = -201339  # 无效的设备名
    DMCSCSL_ERR_InvalidBusType = -201338  # 无效的总线类型
    DMCSCSL_ERR_InvalidCategory = -201337  # 无效的Category
