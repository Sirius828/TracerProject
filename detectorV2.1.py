...
class MainWindow(QMainWindow): # 继承QMainWindow类，是程序的主窗口
    ...
    def setupToolbar(self):
        ...
        # 添加帮助按钮
        help_action = QAction(QIcon("C:/Users/17106/Desktop/pythonProject/icon/help.png"), "帮助", self)
        help_action.triggered.connect(self.show_about)
        self.toolbar.addAction(help_action)

    def addSmartActions(self):
        # 智能添加通道按钮 - 根据模式自动选择功能
        add_channel_act = QAction(QIcon("C:/Users/17106/Desktop/pythonProject/icon/plus.png"), "添加通道/数据", self)
        add_channel_act.triggered.connect(self.smartAddChannel)
        self.toolbar.addAction(add_channel_act)
        
        # 智能四象限探测器按钮 - 根据模式自动选择功能
        add_quadrant_act = QAction(QIcon("C:/Users/17106/Desktop/pythonProject/icon/windows.png"), "四象限探测器", self)
        add_quadrant_act.triggered.connect(self.smartAddQuadrant)
        self.toolbar.addAction(add_quadrant_act)
        
        # 采集/播放按钮 - 只在模拟输入模式可用
        self.acquisition_act = QAction(QIcon("C:/Users/17106/Desktop/pythonProject/icon/finish-flag.png"), "启动采集", self)
        self.acquisition_act.triggered.connect(self.toggle_acquisition)
        self.toolbar.addAction(self.acquisition_act)
        
        # 分析/算法按钮 - 通用功能
        add_algorithm_act = QAction(QIcon("C:/Users/17106/Desktop/pythonProject/icon/sensor.png"), "添加算法", self)
        add_algorithm_act.triggered.connect(self.smartAddAlgorithm)
        self.toolbar.addAction(add_algorithm_act)
        
        # 添加表达式图表按钮 - 两种模式都可用
        expression_chart_act = QAction(QIcon("C:/Users/17106/Desktop/pythonProject/icon/code.png"), "表达式图表", self)
    ...
    def show_dock(self):
        if (self.project_dock.isHidden()):
            ...
            self.project_dock.raise_()  # 如果已经显示，则将其置顶    # ============= 帮助操作 =============    def show_about(self):
        """显示关于对话框"""
        about_text = """
        <h2>Tracer 电压监测应用</h2>
        <p><b>开发者:</b> Voltage Monitoring Team</p>
        ...
        <br>
        <p>© 2025 Voltage Monitoring Solutions. All rights reserved.</p>
        """
    ...
    def show_operation_guide(self):
        ...
        <h3>🚀 快速开始</h3>
        <ol>
        <li><strong>创建新工程:</strong> 文件 → 新建工程</li>
        ...
        <li>从可用通道列表中选择需要监测的通道</li>
        <li>配置通道属性（采样率、量程等）</li>
        <li>点击确定添加通道到项目中</li>
        </ul>
        
        <h4>通道配置</h4>
        <ul>
        <li>右键点击通道可以修改通道属性</li>
        <li>双击通道名称可以重命名</li>
        <li>支持批量配置多个通道</li>
        </ul>
        ...
        <h4>自动配置</h4>
        <ul>
        <li>当项目中有4个通道时，系统会自动提示配置四象限探测器</li>
        <li>系统会智能识别通道命名模式并自动分配象限</li>
        <li>支持多种命名格式：A1-A4、Ch1-Ch4、X1-X4等</li>
        </ul>
